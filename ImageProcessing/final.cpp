...
void detectTennisBall()
{
  while(1)
  {
    sleep(3);
    interProcComp.write("Ready", ipPort);
    data = interProcComp.read(15, 20000);
    int dat = A.updateDat(data, H);
    if(dat == -1){
      kill();
      break;
    }
